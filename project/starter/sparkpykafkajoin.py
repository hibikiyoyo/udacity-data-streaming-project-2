...
# |[7B 22 73 74 61 7...|
# +--------------------+
zSetDecodedEntriesStreamingDF = encodedCustomerDF.withColumn("customer", unbase64(encodedCustomerDF.encodedCustomer).cast("string"))
...
zSetDecodedEntriesStreamingDF.withColumn("customer", from_json("customer", customerJSONschema)).select(col('customer.*')).createOrReplaceTempView("CustomerRecords")
...
emailAndBirthDayStreamingDF = spark.sql("select * from CustomerRecords where customerName is not null AND email is not null AND phone is not null AND birthDay is not null")
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
stediEventStreamingDF = stediEventRawStreamingDF.selectExpr("cast(value as string) value")
...
stediEventStreamingDF.withColumn("value", from_json("value", stediEventSchema))\
    .select(col("value.*"))\
