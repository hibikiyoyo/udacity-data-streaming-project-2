...
redisServerStreamingDF.withColumn("value", from_json("value", kafkaRedisSchema)).select(col("value.*")).createOrReplaceTempView("RedisSortedSet")
...
decodedCustomerDF.withColumn("customer", from_json("customer", customerJsonSchema)).select(col('customer.*')).createOrReplaceTempView("CustomerRecords")
